function setView(v){view=v;document.querySelectorAll('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.view===v));$('#fab').hidden=false;if(v==='home')home();if(v==='dogs')allDogs();if(v==='walks')walkingCalendar();if(v==='sitting')sittingCalendar();if(v==='route')route();scrollTo(0,0)}
...
function addWalkDateRow(x={id:'',date:'',time:''}){const r=$('#walk-date-row-template').content.firstElementChild.cloneNode(true);r.dataset.id=x.id||uid();r.querySelector('.walk-date').value=x.date||'';r.querySelector('.walk-date-time').value=x.time||'';r.querySelector('.remove-walk-date').onclick=()=>r.remove();walkDateRows.append(r)}
function readWalkDates(){return [...walkDateRows.querySelectorAll('.walk-date-row')].map(r=>({id:r.dataset.id||uid(),date:r.querySelector('.walk-date').value,time:r.querySelector('.walk-date-time').value})).filter(x=>x.date).sort((a,b)=>a.date.localeCompare(b.date)||(a.time||'99:99').localeCompare(b.time||'99:99'))}
...
function editDog(d=null){
  $('#dog-form').reset();walkSlots.innerHTML='';walkDateRows.innerHTML='';sitSlots.innerHTML='';sitDateRows.innerHTML='';
...
document.addEventListener('click',e=>{
  ...
  if(e.target.closest('[data-walk-calendar-prev]'))return changeWalkingMonth(-1);
  if(e.target.closest('[data-walk-calendar-next]'))return changeWalkingMonth(1);
  const walkCal=e.target.closest('[data-walk-calendar-date]');if(walkCal){walkingSelectedDate=walkCal.dataset.walkCalendarDate;return walkingCalendar()}
  if(e.target.closest('[data-add-walking-booking]'))return openWalkingBooking(walkingSelectedDate);
...
$('#add-walk-date').onclick=()=>addWalkDateRow({date:walkingSelectedDate||localISO(),time:''});
...
$('#save-walking-booking').onclick=()=>saveWalkingBooking();
